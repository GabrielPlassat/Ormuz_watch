...
# Import du script de mise à jour automatique
...
AUTO_FILE    = "data/auto_scores.json"
...
def main():
    ...
    # ── Panneau de mise à jour automatique
    with st.container():
        ua1, ua2, ua3 = st.columns([2, 2, 3])
        ...
        with ua1:
            btn_update = st.button(
                ...
                disabled=not UPDATER_AVAILABLE,
                help="Lance GDELT + yFinance et recalcule les 20 événements"
            )

        with ua2:
            btn_apply = st.button(
                "💡 Appliquer les suggestions auto",
                use_container_width=True,
                disabled=not auto_scores,
                help="Importe les niveaux détectés automatiquement (ne rétrograde jamais un statut manuel)"
            )
        ...
        with ua3:
            if auto_scores:
                ...
                n_detected = sum(1 for v in auto_scores.values() if v.get("auto_status", 0) > 0)
                st.caption(
                    f"📡 Dernière analyse : **{ts_fmt}**  ·  "
                    f"**{n_detected}** événement(s) détecté(s) automatiquement"
                )
            else:
                st.caption("📡 Aucune analyse disponible — cliquer sur **Mettre à jour les sources**")

        # ── Déclenchement update
        if btn_update:
            if UPDATER_AVAILABLE:
                with st.spinner("Interrogation GDELT + yFinance en cours… (~45s)"):
                    try:
                        auto_scores = updater.run()
                        st.success(f"✅ Analyse terminée — {len(auto_scores)} événements mis à jour")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Erreur lors de la mise à jour : {e}")
            else:
                st.error("Script update_tripwires.py introuvable dans scripts/")

        # ── Application des suggestions
        if btn_apply and auto_scores:
            ...
            n_changes = sum(
                1 for eid in EVENTS
                if merged.get(eid, {}).get("status", 0) != load_state().get(eid, {}).get("status", 0)
            )
            st.success(f"✅ {sum(1 for v in auto_scores.values() if v.get('auto_status',0)>0)} suggestion(s) appliquée(s)")
            st.rerun()
        ...
        for cat, eids in cats.items():
            with st.expander(f"📁 {cat}", expanded=False):
                for eid in eids:
                    ...
                    # Badge auto-score si disponible
                    if eid in auto_scores:
                        a = auto_scores[eid]
                        a_st = a.get("auto_status", 0)
                        a_badges = {0:"⚪", 1:"🟡", 2:"🟠", 3:"🔴"}
                        a_sig = a.get("signal", "")[:70]
                        st.caption(
                            f"{a_badges[a_st]} **Auto** : {a_sig}  "
                            f"<span style='color:#334155'>({a.get('source','?')})</span>",
                            unsafe_allow_html=True
                        )
                        # Affiche les snippets GDELT si disponibles
                        snippets = a.get("snippets", [])
                        if snippets and a_st > 0:
                            with st.expander("📰 Articles détectés", expanded=False):
                                for s in snippets:
                                    st.caption(f"• {s}")
