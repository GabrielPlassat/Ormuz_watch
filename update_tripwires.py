...
# CONFIG
# ─────────────────────────────────────────────────────────────────────────
# Chemin absolu depuis le script — fonctionne depuis n'importe quel CWD
        ...
        for ticker in TANKER_TICKERS:
            ...
            for attempt in range(3):
                try:
                    ...
                    wait = 5 * (attempt + 1)
                    log.warning(f"  {ticker} rate-limit — retry dans {wait}s")
                    time.sleep(wait)
            ...
            time.sleep(2)   # pause entre chaque ticker
...
def query_gdelt(query: str, max_retries: int = 3) -> tuple[int, list[str]]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Gérer 429 (rate-limit)
            if r.status_code == 429:
                wait = GDELT_PAUSE * (3 ** attempt)
                log.warning(f"  GDELT 429 — attente {wait:.0f}s (tentative {attempt+1}/{max_retries})")
                time.sleep(wait)
                continue
            # Gérer 403 (IP bloquée / proxy)
            if r.status_code == 403:
                log.warning(f"  GDELT 403 — IP bloquée ou proxy réseau")
                return -1, []
            r.raise_for_status()
            # Vérifier que la réponse n'est pas vide
            text = r.text.strip()
            if not text or text.startswith("Host not"):
                log.warning(f"  GDELT réponse vide/bloquée pour '{query[:40]}'")
                return -1, []
            # Parser JSON
            try:
                data = r.json()
            except Exception:
                log.warning(f"  GDELT JSON invalide pour '{query[:40]}': '{text[:80]}'")
                return -1, []
            ...
            if attempt < max_retries - 1:
                ...
                log.warning(f"  GDELT échec définitif pour '{query[:40]}': {e}")
                return -1, []
    return -1, []
...
def run() -> dict:
    ...
    # ── 1. Événements GDELT ──────────────────────────────────────────────
    # Activé uniquement si CI=true (GitHub Actions) — IP cloud Streamlit trop bloquée
    # En local ou Streamlit Cloud → statut inchangé (conserve la dernière valeur connue)
    run_gdelt = os.environ.get("CI", "").lower() == "true"

    if not run_gdelt:
        log.info("\n[GDELT] Non exécuté (IP Streamlit Cloud bloquée). Statuts conservés.")
        for eid, cfg in GDELT_EVENTS.items():
            existing_st = load_existing_status(eid)
            auto_scores[eid] = {
                "auto_status": existing_st,
                "signal":      "GDELT actif uniquement via GitHub Actions (toutes les heures)",
                "source":      "cache",
                "snippets":    [],
                "updated_at":  datetime.now().isoformat(),
            }
    else:
        log.info(f"\n[GDELT] Actif (GitHub Actions) — {len(GDELT_EVENTS)} événements, fenêtre {GDELT_TIMESPAN}")

        # 4 requêtes groupées au lieu de 18 (respecte le rate-limit GDELT)
        # Requêtes courtes et simples — GDELT fait une recherche AND implicite
        # Les OR longs provoquaient des erreurs d'encodage et des réponses vides
        GDELT_GROUPS = {
            "physical_strait": {
                "eids":  ["E01", "E02", "E03"],
                "query": "Iran tanker Hormuz seized",
                "label": "Physique — Détroit",
            },
            "infra_military": {
                "eids":  ["E04", "E05", "E06", "E13", "E15"],
                "query": "Saudi Arabia oil attack Israel Iran strike",
                "label": "Infrastructure + Militaire",
            },
            "market_shipping": {
                "eids":  ["E07", "E10", "E11", "E14", "E18", "E19"],
                "query": "Hormuz shipping tanker reroute Houthi insurance",
                "label": "Marchés + Fret + AIS",
            },
            "institutional": {
                "eids":  ["E12", "E16", "E17", "E20"],
                "query": "Iran oil sanctions IEA reserve release",
                "label": "Institutionnel + Systémique",
            },
        }

        # Lancer les 4 requêtes groupées
        group_results: dict[str, tuple[int, list]] = {}
        for gid, grp in GDELT_GROUPS.items():
            log.info(f"  → {grp['label']}")
            n, snippets = query_gdelt(grp["query"])
            group_results[gid] = (n, snippets)
            log.info(f"     {n} articles trouvés")
            time.sleep(GDELT_PAUSE)

        # Distribuer vers chaque événement
        for eid, cfg in GDELT_EVENTS.items():
            grp_id = next(
                (gid for gid, grp in GDELT_GROUPS.items() if eid in grp["eids"]),
                None,
            )
            if grp_id is None:
                log.info(f"  → {eid} (individuel) : {cfg['label']}")
                n, snippets = query_gdelt(cfg["query"])
                time.sleep(GDELT_PAUSE)
            else:
                n, snippets = group_results[grp_id]

            status = articles_to_status(n, cfg["thresholds"]) if n >= 0 else 0
            auto_scores[eid] = {
                "auto_status": status,
                "signal":      (f"{n} articles/24h · seuils {cfg['thresholds']}"
                                if n >= 0 else "Erreur API GDELT"),
                "source":      "GDELT",
                "snippets":    snippets if n > 0 else [],
                "updated_at":  datetime.now().isoformat(),
            }

        n_actifs = sum(1 for v in auto_scores.values()
                       if v.get("source") == "GDELT" and v.get("auto_status", 0) > 0)
        log.info(f"  GDELT terminé — {n_actifs} événement(s) actif(s)")
