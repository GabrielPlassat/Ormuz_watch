...
def run() -> dict:
    ...
    # ── 1. Événements GDELT — requêtes groupées (4 au lieu de 18) ──────────
    # Regrouper par thème pour réduire le nombre d'appels API et éviter le 429
    GDELT_GROUPS = {
        "physical_strait": {
            "eids":  ["E01", "E02", "E03"],
        ...
        "infra_military": {
            ...
            "query": ("ADNOC attack Fujairah OR Petroline pipeline Saudi attack OR "
                      ...
                      "IRGC naval exercise Hormuz unannounced"),
            "label": "Infrastructure + Militaire",
        },
        "market_shipping": {
            "eids":  ["E07", "E08_proxy", "E10", "E11", "E14", "E18", "E19"],
            "query": ("war risk insurance Gulf Hormuz OR Lloyd JWC exclusion OR "
                      "tanker reroute Cape Good Hope OR Sinopec cancel oil cargo OR "
                      "Houthi attack ship Red Sea OR tanker traffic Hormuz decline OR "
    ...
    for eid, cfg in GDELT_EVENTS.items():
        ...
        if n >= 0:
            status = articles_to_status(n, cfg["thresholds"])
        else:
            status = 0
    ...
    log.info(f"  GDELT terminé — {n_actifs} événement(s) actif(s) détecté(s)")
