...
# Chemin vers la racine du repo (robuste en local, Colab, Streamlit Cloud)
_REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)

try:
    import scripts.update_tripwires as updater
...
STATE_FILE   = os.path.join(_DATA_DIR, "event_states.json")
HISTORY_FILE = os.path.join(_DATA_DIR, "event_history.json")
AUTO_FILE    = os.path.join(_DATA_DIR, "auto_scores.json")
...
def gauge_risk(value, title, subtitle=""):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "bordercolor": BORDER,
            "steps": [
                {"range": [0, 25],   "color": "rgba(20,83,45,0.13)"},
                {"range": [25, 50],  "color": "rgba(120,53,15,0.13)"},
                {"range": [50, 75],  "color": "rgba(124,45,18,0.13)"},
                {"range": [75, 100], "color": "rgba(69,10,10,0.20)"},
            ],
        }
    ))
    fig.update_layout(
...
def gauge_decoupling(value):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "bordercolor": BORDER,
            "steps": [
                {"range": [-100, -30], "color": "rgba(12,74,110,0.13)"},
                {"range": [-30, 15],   "color": "rgba(20,83,45,0.13)"},
                {"range": [15, 45],    "color": "rgba(120,53,15,0.13)"},
                {"range": [45, 100],   "color": "rgba(69,10,10,0.20)"},
            ],
        }
    ))
    fig.update_layout(
...
# AUTO SCORES
# ─────────────────────────────────────────────────────────────────────────
def load_auto_scores() -> dict:
    """Charge les scores automatiques depuis data/auto_scores.json."""
    if os.path.exists(AUTO_FILE):
        with open(AUTO_FILE) as f:
            return json.load(f)
    return {}

def apply_auto_scores(state: dict, auto_scores: dict) -> dict:
    """
    Fusionne les scores auto dans l'état manuel.
    Règle : le score auto surclasse le statut manuel UNIQUEMENT si
    auto_status > statut manuel actuel (on ne rétrograde jamais manuellement).
    L'analyste garde toujours la main via le slider.
    """
    merged = {eid: dict(v) for eid, v in state.items()}
    for eid, auto in auto_scores.items():
        if eid not in merged:
            continue
        auto_st = auto.get("auto_status", 0)
        cur_st  = merged[eid].get("status", 0)
        if auto_st > cur_st:
            merged[eid]["status"]     = auto_st
            merged[eid]["notes"]      = (
                f"[AUTO — {auto.get('source','?')}] {auto.get('signal','')} "
                + (merged[eid].get("notes") or "")
            ).strip()
            merged[eid]["updated_at"] = datetime.now().isoformat()
    return merged
...
def main():
    ...
    # ── Charger l'état + scores auto
    if "event_state" not in st.session_state:
        st.session_state.event_state = load_state()
    state       = st.session_state.event_state
    auto_scores = load_auto_scores()

    # ── Mise à jour automatique des sources (TTL 1h)
    @st.cache_data(ttl=3600, show_spinner=False)
    def _auto_update():
        """Lance GDELT + yFinance si le cache est expiré (1h)."""
        if UPDATER_AVAILABLE:
            try:
                return updater.run()
            except Exception:
                pass
        return load_auto_scores()   # fallback : données déjà sur disque

    with st.spinner("📡 Mise à jour des sources en cours…"):
        auto_scores = _auto_update()

    # ── Application automatique des suggestions (ne rétrograde jamais le manuel)
    if auto_scores:
        merged = apply_auto_scores(state, auto_scores)
        if merged != state:
            save_state(merged)
            st.session_state.event_state = merged
            state = merged

    # ── Statut de la dernière analyse
    if auto_scores:
        ts        = next(iter(auto_scores.values())).get("updated_at", "")
        ts_fmt    = ts[:16].replace("T", " ") if ts else "—"
        n_evt     = sum(1 for v in auto_scores.values() if v.get("auto_status", 0) > 0)
        st.caption(f"📡 Sources actualisées : **{ts_fmt}** · **{n_evt}** événement(s) actif(s) détecté(s) automatiquement")
    else:
        st.caption("📡 Sources indisponibles (GDELT / yFinance)")

    st.divider()

    # ── Panneau RSS : synthèse des signaux actifs
    rss_actifs = {
        eid: v for eid, v in auto_scores.items()
        if v.get("auto_status", 0) > 0 and v.get("source") in ("RSS", "GDELT")
        and v.get("snippets")
    }
    # Dédupliquer : un seul bloc par groupe de snippets identiques
    seen_snippets = set()
    rss_groupes   = []
    for eid, v in sorted(rss_actifs.items(), key=lambda x: -x[1]["auto_status"]):
        key = tuple(v["snippets"])
        if key not in seen_snippets:
            seen_snippets.add(key)
            rss_groupes.append((eid, v))

    if rss_groupes:
        with st.expander(
            f"📰 {sum(len(v['snippets']) for _,v in rss_groupes)} article(s) RSS détectés "
            f"— {len(rss_groupes)} groupe(s) actif(s)",
            expanded=True
        ):
            for eid, v in rss_groupes:
                a_st      = v["auto_status"]
                sev_color = {1:"#92400e", 2:"#c2410c", 3:"#991b1b"}.get(a_st, "#334155")
                sev_lbl   = {1:"🟡 Veille", 2:"🟠 Alerte", 3:"🔴 Déclenché"}.get(a_st, "")
                ev_ids    = [e for e,ev in EVENTS.items()
                             if auto_scores.get(e, {}).get("snippets") == v["snippets"]
                             and auto_scores.get(e, {}).get("auto_status", 0) > 0]
                ev_labels = " · ".join(ev_ids)
                ts        = v.get("updated_at","")[:16].replace("T"," ")

                st.markdown(
                    f'<div style="border-left:3px solid {sev_color};padding:6px 10px;'
                    f'background:{sev_color}18;border-radius:4px;margin-bottom:8px">'
                    f'<span style="font-size:12px;font-weight:600">{sev_lbl} — {ev_labels}</span>'
                    f'<span style="color:#64748b;font-size:10px;float:right">{ts}</span>'
                    f'</div>',
                    unsafe_allow_html=True
                )
                for s in v["snippets"]:
                    if "(" in s and s.endswith(")"):
                        ...
                        src2 = s[idx+1:-1]
                        st.markdown(
                            f'<div style="padding:3px 10px;font-size:12px">'
                            f'📄 {text} '
                            f'<span style="color:#64748b;font-size:10px">— {src2}</span>'
                            f'</div>',
                            unsafe_allow_html=True
                        )
                    else:
                        st.markdown(f'<div style="padding:3px 10px;font-size:12px">📄 {s}</div>',
                                    unsafe_allow_html=True)
    ...
    if len(history) >= 2:
        ...
        st.plotly_chart(chart_history(history))
        st.divider()
    ...
    with col_chart:
        st.plotly_chart(chart_contributions(state))
        ...
        for cat, eids in cats.items():
            with st.expander(f"📁 {cat}", expanded=False):
                for eid in eids:
                    ...
                    # Badge auto-score + snippets RSS
                    if eid in auto_scores:
                        a    = auto_scores[eid]
                        a_st = a.get("auto_status", 0)
                        src_label = a.get("source", "?")
                        a_badges  = {0: "⚪", 1: "🟡", 2: "🟠", 3: "🔴"}
                        sev_color = {0: "#475569", 1: "#92400e", 2: "#c2410c", 3: "#991b1b"}
                        a_sig = a.get("signal", "")[:80]
                        ts    = a.get("updated_at", "")[:16].replace("T", " ")

                        # Ligne de statut auto
                        st.markdown(
                            f'<div style="background:{sev_color.get(a_st,"#1e2d3d")}22;'
                            f'border-left:3px solid {sev_color.get(a_st,"#334155")};'
                            f'border-radius:4px;padding:5px 9px;margin:3px 0;font-size:11px">'
                            f'{a_badges[a_st]} <b>Auto ({src_label})</b> : {a_sig}'
                            f'<span style="color:#64748b;float:right">{ts}</span>'
                            f'</div>',
                            unsafe_allow_html=True
                        )

                        # Snippets RSS : affichés directement si alerte active
                        snippets = a.get("snippets", [])
                        if snippets and a_st > 0:
                            exp_label = f"📰 {len(snippets)} article(s) source{'s' if len(snippets)>1 else ''}"
                            with st.expander(exp_label, expanded=(a_st >= 2)):
                                for s in snippets:
                                    # Séparer titre et source "(source)"
                                    if "(" in s and s.endswith(")"):
                                        idx  = s.rfind("(")
                                        text = s[:idx].strip()
                                        src2 = s[idx+1:-1]
                                        st.markdown(
                                            f'<div style="padding:4px 0;border-bottom:1px solid #1e2d3d;font-size:12px">'
                                            f'📄 {text}'
                                            f'<span style="color:#64748b;font-size:10px"> — {src2}</span>'
                                            f'</div>',
                                            unsafe_allow_html=True
