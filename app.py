    ...
    with tab_ui:
        ...
        with right:
            if series_map:
                ...
                # add_vline avec x en string (pd.Timestamp bug plotly + annotations séparées)
                closure_str = CLOSURE_DATE.strftime("%Y-%m-%d")
                reopen_str  = reopen_date.strftime("%Y-%m-%d")
                fig.add_vline(x=closure_str, line=dict(dash="dash", color="#ef4444", width=1.1))
                fig.add_vline(x=reopen_str,  line=dict(dash="dash", color=scen["color"], width=1.1))
                fig.add_annotation(x=closure_str, y=1, xref="x", yref="paper",
                                   text="Fermeture", showarrow=False,
                                   font=dict(size=9, color="#ef4444"),
                                   xanchor="left", yanchor="top", bgcolor="white", opacity=0.8)
                fig.add_annotation(x=reopen_str, y=1, xref="x", yref="paper",
                                   text=scen["label"], showarrow=False,
                                   font=dict(size=9, color=scen["color"]),
                                   xanchor="right", yanchor="top", bgcolor="white", opacity=0.8)
