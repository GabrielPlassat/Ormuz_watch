        ...
        for inst in instruments:
            ...
            if ico in ("🔴","🟠") and cur is not None:
                alert_rows.append({"Signal": ico, "Cat.": cat.split()[-1], "Indicateur": inst["label"],
                                   ...
                                   "Note": inst["note"]})
    if alert_rows:
        st.dataframe(pd.DataFrame(alert_rows).sort_values("Signal"), hide_index=True)
    else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.plotly_chart(gantt_chart(make_gantt(scen_days), p["color"], p["label"]))
    ...
    st.plotly_chart(risk_chart(scen_days))
